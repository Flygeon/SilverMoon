//! 窗口层：`WebviewWindow` / `WebviewWindowBuilder` / `WebviewUrl` / `Cookie`。
//!
//! 窗口实体全部由 Electron 宿主机持有（`BrowserWindow`），这里只是按 label
//! 定向的句柄，方法一律转发给宿主机。
//!
//! # 与 Tauri 的一处行为差异（`on_navigation`）
//!
//! Tauri 的 `on_navigation` 回调是**同步**的，返回 `false` 可以当场拦掉这次导航。
...
use crate::app::{AppHandle, AppInner, Error, Result};
use crate::host::{self, HostOp};

/// 导航拦截回调表（label → 回调）。
pub type NavigationGuard = Arc<dyn Fn(&Url) -> bool + Send + Sync>;

/// 进程级回调表。`WebviewWindowBuilder::on_navigation` 注册，宿主回调时查询。
...
impl WebviewUrl {
    fn to_payload(&self) -> Value {
        match self {
            WebviewUrl::External(url) => json!({ "kind": "external", "url": url.as_str() }),
            WebviewUrl::App(path) => json!({
                "kind": "app",
...
/// 等价 `tauri::WebviewWindow`。`Clone + Send + Sync`，可跨线程持有。
#[derive(Clone)]
...
impl WebviewWindow {
    ...
    ///
    /// 与 Tauri 一致：回调类型是 `Fn`（可能被多次调用），因此内部把闭包放进
...
impl WebviewWindowBuilder {
    ...
    pub fn build(self) -> Result<WebviewWindow> {
        ...
        let window = WebviewWindow::adopt(&self.inner, &self.label);
        self.inner
            .windows
            .lock()
