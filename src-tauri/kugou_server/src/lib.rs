//! kugou_server — Rust 实现的酷狗 API 客户端（vendored from MD3Music）。
//!
//! 上游形态：在进程内启动 tiny_http 服务，监听 127.0.0.1 上的随机端口
//! （端口号回传给 Dart），行为等价于 kugou_api_server/server.js。
//!
//! 本项目（LumiLuna）以 path 依赖方式链接本 crate，**不使用 HTTP 服务端**，
//! 改由 [`bridge`] 进程内直调同一张路由表（见 bridge.rs 顶部说明）。因此桌面
//! 构建无需 cdylib 与 JNI 符号，下方 FFI / JNI 导出仅为与上游保持可比对而保留。
//!
//! 对外接口：
//!  - 本项目用：`bridge::call`
