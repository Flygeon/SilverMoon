...
/// 路由表（HTTP 与 bridge 进程内调用共用）。
pub(crate) fn routes() -> &'static Vec<(&'static str, ModuleFn)> {
    ROUTES.get_or_init(|| {
...
pub(crate) fn session_guid() -> String {
    DeviceConfig::instance().get_guid()
}

pub(crate) fn session_mid() -> String {
    DeviceConfig::instance().get_mid()
}

pub(crate) fn session_dev() -> String {
    DeviceConfig::instance().get_server_dev().to_uppercase()
}

/// 供 [`crate::bridge`] 使用：登记持久化目录。
/// 上游在 `start()` 里设置 `DATA_DIR`（`persist_registered_device` 落盘时读它）；
/// 本项目不起 HTTP 服务，改由 bridge::init 显式登记，行为保持一致。
pub(crate) fn set_data_dir(dir: String) {
...
pub(crate) fn persist_registered_device(body: &BodyValue) {
    let j = body.to_json();
...
pub(crate) fn prefix_match(route: &str, path: &str) -> bool {
    if path == route {
        return true;
    }
    path.len() > route.len()
...
pub(crate) fn parse_express_cookies(cookie: &str, out: &mut Map<String, Value>) {
...
pub(crate) fn cookie_to_json(s: &str) -> Value {
    let mut obj = Map::new();
    for part in s.split(';') {
