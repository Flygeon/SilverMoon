//! silvermoon-ipc 的宏实现。
//!
//! 目标：让 `backend/src` 下的 156 个业务命令**一行都不用改**。为此本 crate 复刻了
//! 提供三个宏：
//!
//! * `#[command]` —— 保留原函数定义，额外生成一个 `__silvermoon_register_<fn_name>()`，
//!   返回一条 [`CommandDef`] 路由记录。
//! * `generate_handler![a::b, c::d]` —— 把路径列表展开成 `__silvermoon_register_b()` 的调用列表，
...
//! | 形态 | 判定依据 | 生成代码 |
//! |---|---|---|
//! | `Host` | 类型末段为 `Host`（含 `silvermoon_ipc::Host`） | 由 `Ctx` 注入，不占 JSON 键 |
...
/// 取类型路径的末段标识符（`silvermoon_ipc::Host` → `Host`）。
...
fn expand_command(func: &ItemFn) -> syn::Result<TokenStream2> {
    ...
    for input in &func.sig.inputs {
        let FnArg::Typed(pat_type) = input else {
            ...
            return Err(syn::Error::new(
                Span::call_site(),
                "silvermoon_ipc::command 不支持 self 接收者",
            ));
        };
        let Pat::Ident(pat_ident) = &*pat_type.pat else {
            return Err(syn::Error::new_spanned(
                &pat_type.pat,
                "silvermoon_ipc::command 只支持简单标识符形参",
            ));
        };
    ...
    let body = if returns_result {
        quote! {
            match #call {
                ::std::result::Result::Ok(__v) => ::silvermoon_ipc::__internal::to_command_value(&__v),
        ...
        quote! {
            let __ret = #call;
            ::silvermoon_ipc::__internal::to_command_value(&__ret)
        }
    };

    let register_ident = format_ident!("__silvermoon_register_{}", name);
    ...
    Ok(quote! {
        ...
        pub fn #register_ident() -> ::silvermoon_ipc::__internal::CommandDef {
            ::silvermoon_ipc::__internal::CommandDef::new(#name_str, |__ctx, __args| {
                let __ctx: ::silvermoon_ipc::__internal::Ctx = *__ctx;
...
pub fn generate_handler(input: TokenStream) -> TokenStream {
    ...
    for path in paths {
        ...
        last.ident = format_ident!("__silvermoon_register_{}", last.ident);
    ...
    quote! {
        {
            ...
            ::silvermoon_ipc::__internal::Context::new(::silvermoon_ipc::__internal::ContextConfig {
...
/// 桌面端空操作：仅为 `#[cfg_attr(mobile, silvermoon_ipc::mobile_entry_point)]` 存在。
#[proc_macro_attribute]
