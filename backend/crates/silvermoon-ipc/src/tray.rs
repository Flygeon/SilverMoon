...
use crate::app::{Error, Host, Result};
...
type MenuHandler = Arc<dyn Fn(&Host, MenuEvent) + Send + Sync>;
...
static APP_HANDLE: OnceLock<Host> = OnceLock::new();
...
impl TrayIconBuilder {
    ...
    where
        F: Fn(&Host, MenuEvent) + Send + Sync + 'static,
    {
    ...
    /// 创建托盘
    pub fn build(self, app: &Host) -> Result<TrayIcon> {
