...
//!
//! 这层是整块兼容层的地基：业务代码里的 `app: silvermoon_ipc::Host`、
...
/// 兼容 `silvermoon_ipc::Error`。业务代码只用到 [`Error::AssetNotFound`]，
/// 其余变体供兼容层内部使用。
#[derive(Debug)]
pub enum Error {
...
/// 等价 `silvermoon_ipc::State`：托管状态的只读借用。
...
/// 等价 `silvermoon_ipc::path::PathResolver`。只实现业务代码用到的两个目录。
...
// ---------------------------------------------------------------------------
// HostApi / EventEmitter
// ---------------------------------------------------------------------------

/// 等价 `silvermoon_ipc::HostApi`。
pub trait HostApi {
    /// 内部共享状态
    #[doc(hidden)]
    ...
    fn get_webview_window(&self, label: &str) -> Option<Window> {
        ...
        let win = Window::adopt(inner, label);
        inner
            .windows
            .lock()
...
/// 等价 `silvermoon_ipc::EventEmitter`。
...
/// 等价 `silvermoon_ipc::Host`。可 `Clone`、`Send`、`Sync`、`'static`，常用于跨线程持有。
#[derive(Clone)]
pub struct Host {
...
impl EventEmitter for Host {
    fn app_inner(&self) -> &Arc<AppInner> {
        &self.inner
    }
}

/// 等价 `silvermoon_ipc::App`：`setup` 闭包收到的对象。
pub struct App {
    inner: Arc<AppInner>,
    handle: Host,
}
...
impl App {
    /// 取句柄，用于跨线程持有或传给命令函数
    pub fn handle(&self) -> &Host {
        &self.handle
    }
}

impl HostApi for App {
    fn app_inner(&self) -> &Arc<AppInner> {
        &self.inner
    }
}

impl EventEmitter for App {
    fn app_inner(&self) -> &Arc<AppInner> {
        &self.inner
    }
}
...
// ---------------------------------------------------------------------------
// Context（替代 silvermoon_ipc::generate_context!）
// ---------------------------------------------------------------------------
...
/// 等价 `silvermoon_ipc::Builder`。
///
/// 宿主是 Electron：不注册插件，插件能力由宿主机承担；
            ...
            if let Some(labels) = value.as_array() {
                ...
                for label in labels.iter().filter_map(|v| v.as_str()) {
                    guard
                        ...
                        .or_insert_with(|| Window::adopt(&inner, label));
                }
            }
        }
