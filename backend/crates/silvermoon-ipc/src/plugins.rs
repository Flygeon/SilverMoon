//! 宿主能力的最小实现：打开文件 / 定位文件 / 全局热键。
//!
//! 这两项在业务代码里各只有一个调用点（`commands/extension.rs`），因此只
//! 提供所需的最小 API；真正的系统调用由 Electron 宿主执行。
...
/// `silvermoon_ipc::OpenerError` 的替身。
#[derive(Debug)]
...
/// 等价 `silvermoon_ipc::OpenerExt`。
pub trait OpenerExt {
    /// 取 opener
    fn opener(&self) -> Opener;
}

impl OpenerExt for Host {
    fn opener(&self) -> Opener {
        Opener {
...
pub(crate) type ShortcutHandler = Arc<dyn Fn(&Host, &Shortcut, ShortcutEvent) + Send + Sync>;
...
impl GlobalShortcut {
    ...
    where
        S: Into<Shortcut>,
        F: Fn(&Host, &Shortcut, ShortcutEvent) + Send + Sync + 'static,
    {
...
/// 等价 `silvermoon_ipc::GlobalShortcutExt`。
pub trait GlobalShortcutExt {
    /// 取全局热键注册器
    fn global_shortcut(&self) -> GlobalShortcut;
}

impl GlobalShortcutExt for Host {
    fn global_shortcut(&self) -> GlobalShortcut {
        GlobalShortcut {
            inner: Arc::clone(&self.inner),
        }
