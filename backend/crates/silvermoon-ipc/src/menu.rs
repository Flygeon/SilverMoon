...
impl PredefinedMenuItem {
    /// 分隔符
    pub fn separator(_app: &Host) -> Result<PredefinedMenuItem> {
...
impl Submenu {
    /// 新建子菜单
    pub fn new(_app: &Host, text: impl Into<String>, enabled: bool) -> Result<Submenu> {
        Ok(Submenu {
            text: text.into(),
            enabled,
...
impl Menu {
    /// 新建空菜单
    pub fn new(_app: &Host) -> Result<Menu> {
        Ok(Menu {
            items: Mutex::new(Vec::new()),
        })
    }
