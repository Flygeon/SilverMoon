...
use silvermoon_ipc::{
    EventEmitter, GlobalShortcutExt, HostApi, Shortcut, ShortcutState, WindowBuilder, WindowUrl,
};
...
fn ext_base_dir(app: &silvermoon_ipc::Host) -> Result<PathBuf, String> {
    let dir = app
        .path()
        .app_data_dir()
...
fn enabled_state_path(app: &silvermoon_ipc::Host) -> Result<PathBuf, String> {
    let dir = app
        .path()
        .app_data_dir()
        .map_err(|e| format!("无法定位 app data 目录：{e}"))?;
...
fn load_enabled_state(app: &silvermoon_ipc::Host) -> HashMap<String, bool> {
...
pub fn setup(app: &silvermoon_ipc::Host) -> silvermoon_ipc::Result<()> {
...
fn load_extensions(app: &silvermoon_ipc::Host) -> Result<(), String> {
...
fn spawn_engine(app: &silvermoon_ipc::Host, id: &str) -> Result<u16, String> {
...
fn stop_engine(app: &silvermoon_ipc::Host, id: &str) {
    let state = app.state::<ExtState>();
    let mut map = state.map.lock().unwrap();
    if let Some(ext) = map.get_mut(id) {
...
pub fn ext_list(app: silvermoon_ipc::Host) -> Vec<ExtInfo> {
    let state = app.state::<ExtState>();
    let map = state.map.lock().unwrap();
    map.values()
        .map(|e| ExtInfo {
...
#[silvermoon_ipc::command]
pub async fn ext_install(app: silvermoon_ipc::Host, source: ExtSource) -> Result<ExtInfo, String> {
    silvermoon_ipc::rt::spawn_blocking(move || {
        let base = ext_base_dir(&app)?;
        let _ = std::fs::create_dir_all(&base);
...
/// 卸载扩展：停引擎 + 删目录
#[silvermoon_ipc::command]
pub async fn ext_uninstall(app: silvermoon_ipc::Host, id: String) -> Result<(), String> {
    silvermoon_ipc::rt::spawn_blocking(move || {
        stop_engine(&app, &id);
        let base = ext_base_dir(&app)?;
...
fn register_hotkeys(app: &silvermoon_ipc::Host) {
...
pub fn tray_menu_items(app: &silvermoon_ipc::Host) -> Vec<(String, String)> {
    let mut out = Vec::new();
    let state = app.state::<ExtState>();
    let map = state.map.lock().unwrap();
    for e in map.values() {
...
pub fn handle_tray_event(app: &silvermoon_ipc::Host, id: &str) {
    ...
    if let Some(action) = action {
        if let Some(route) = action.strip_prefix("open:") {
            ...
            silvermoon_ipc::rt::spawn(async move {
