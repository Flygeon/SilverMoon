...
pub fn get_song(app: silvermoon_ipc::Host, file_id: String) -> Result<Song, String> {
...
/// 记录一次播放（历史页用）
#[silvermoon_ipc::command]
pub fn record_play(app: silvermoon_ipc::Host, file_id: String) -> Result<(), String> {
    let state = app.state::<DbState>();
    let conn = state.0.lock().map_err(|e| e.to_string())?;
    conn.execute(
...
/// 收藏 / 取消收藏，返回操作后的状态
#[silvermoon_ipc::command]
pub fn toggle_favorite(app: silvermoon_ipc::Host, file_id: String) -> Result<bool, String> {
    let state = app.state::<DbState>();
    let conn = state.0.lock().map_err(|e| e.to_string())?;
...
/// 收藏列表
#[silvermoon_ipc::command]
pub fn list_favorites(app: silvermoon_ipc::Host) -> Result<Vec<crate::MediaEntry>, String> {
    query_entries(
        app,
...
/// 最近播放（按文件去重，保留最近一次）
#[silvermoon_ipc::command]
pub fn list_history(app: silvermoon_ipc::Host) -> Result<Vec<crate::MediaEntry>, String> {
    query_entries(
        app,
        "SELECT f.id, f.path, f.parent, f.name, f.ext, f.type, f.size, f.mtime, f.scanned_at, f.deleted,
                m.title, m.artist, m.album, m.duration_ms, m.width, m.height, m.codec, m.fps,
...
/// 已从磁盘消失、被软删除的条目（回收站）
#[silvermoon_ipc::command]
pub fn list_trash(app: silvermoon_ipc::Host) -> Result<Vec<crate::MediaEntry>, String> {
    query_entries(
        app,
        "SELECT f.id, f.path, f.parent, f.name, f.ext, f.type, f.size, f.mtime, f.scanned_at, f.deleted,
                m.title, m.artist, m.album, m.duration_ms, m.width, m.height, m.codec, m.fps,
...
/// 清空回收站（仅删索引记录，不动磁盘文件）
#[silvermoon_ipc::command]
pub fn empty_trash(app: silvermoon_ipc::Host) -> Result<usize, String> {
    let state = app.state::<DbState>();
    let conn = state.0.lock().map_err(|e| e.to_string())?;
