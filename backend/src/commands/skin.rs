...
fn skins_dir(app: &silvermoon_ipc::Host) -> Result<PathBuf, String> {
...
fn staging_root(app: &silvermoon_ipc::Host) -> Result<PathBuf, String> {
...
#[silvermoon_ipc::command]
pub fn skin_stage_zip(app: silvermoon_ipc::Host, path: String) -> Result<StagedSkin, String> {
...
#[silvermoon_ipc::command]
pub fn skin_commit(app: silvermoon_ipc::Host, staging: String, id: String) -> Result<(), String> {
...
#[silvermoon_ipc::command]
pub fn skin_abort(app: silvermoon_ipc::Host, staging: String) -> Result<(), String> {
    if !valid_staging(&staging) {
        return Err(format!("非法 staging 令牌：{staging}"));
    }
    let src = staging_root(&app)?.join(&staging);
...
#[silvermoon_ipc::command]
pub fn skin_save(app: silvermoon_ipc::Host, id: String, json: String) -> Result<(), String> {
    if !valid_id(&id) || reserved_name(&id) {
        return Err(format!("非法皮肤 id：{id}"));
    }
    let dir = skins_dir(&app)?;
...
#[silvermoon_ipc::command]
pub fn skin_list(app: silvermoon_ipc::Host) -> Result<Vec<SkinEntry>, String> {
    let dir = skins_dir(&app)?;
    let mut out = Vec::new();
...
#[silvermoon_ipc::command]
pub fn skin_load(app: silvermoon_ipc::Host, id: String) -> Result<LoadedSkin, String> {
    if !valid_id(&id) {
        return Err(format!("非法皮肤 id：{id}"));
    }
...
#[silvermoon_ipc::command]
pub fn skin_delete(app: silvermoon_ipc::Host, id: String) -> Result<(), String> {
    if !valid_id(&id) {
        return Err(format!("非法皮肤 id：{id}"));
    }
    let dir = skins_dir(&app)?.join(&id);
...
/// skins 目录绝对路径：前端用 toAssetUrl 把包内相对路径拼成 asset:// URL
#[silvermoon_ipc::command]
pub fn skin_dir(app: silvermoon_ipc::Host) -> Result<String, String> {
