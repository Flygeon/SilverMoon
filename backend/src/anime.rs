...
/// 通用抓取命令。
///
/// **必须 async + spawn_blocking**：同步 `#[silvermoon_ipc::command]` 会占用事件循环线程，
...
fn rules_dir(app: &silvermoon_ipc::Host) -> Result<std::path::PathBuf, String> {
    let dir = app
        .path()
        .app_data_dir()
...
fn load_rule(app: &silvermoon_ipc::Host, name: &str) -> Result<serde_json::Value, String> {
...
#[silvermoon_ipc::command]
pub fn anime_rules_list(app: silvermoon_ipc::Host) -> Result<Vec<AnimeRuleEntry>, String> {
    let dir = rules_dir(&app)?;
...
fn disabled_state_path(app: &silvermoon_ipc::Host) -> Result<std::path::PathBuf, String> {
    Ok(rules_dir(app)?.join("disabled.json"))
}

/// 读取被禁用的规则名集合（文件缺失/损坏时返回空集，等价于「全部启用」）。
fn read_disabled(app: &silvermoon_ipc::Host) -> std::collections::HashSet<String> {
...
#[silvermoon_ipc::command]
pub fn anime_rules_delete(app: silvermoon_ipc::Host, name: String) -> Result<(), String> {
    let dir = rules_dir(&app)?;
    let path = dir.join(format!("{}.json", sanitize_rule_name(&name)));
    if path.exists() {
...
pub fn anime_history_list(
    state: silvermoon_ipc::State<'_, DbState>,
...
fn ensure_webview(app: &silvermoon_ipc::Host) -> Result<silvermoon_ipc::Window, String> {
    ...
    silvermoon_ipc::WindowBuilder::new(
        app,
        WEBVIEW_LABEL,
        silvermoon_ipc::WindowUrl::External("about:blank".parse().unwrap()),
    )
    .visible(false)
    .initialization_script(INIT_SCRIPT)
    .build()
...
///
/// 注意：`eval_with_callback` 的回调是 **`Fn`，不是 `FnOnce`**（宿主回调协议：
...
/// 只有第一次拿得到 Sender）。
async fn eval_json(webview: &silvermoon_ipc::Window, script: &str) -> Option<String> {
...
async fn log_webview_diag(webview: &silvermoon_ipc::Window, rule_name: &str, waited_ms: u128) {
...
fn stop_webview(webview: &silvermoon_ipc::Window) {
    let _ = webview.eval(
