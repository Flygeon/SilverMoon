...
fn save_persist(app: &silvermoon_ipc::Host) {
...
pub fn kugou_login_status(app: silvermoon_ipc::Host) -> KugouLoginStatus {
    ensure_loaded(&app);
...
/// 拉取二维码 key 与内容
#[silvermoon_ipc::command]
pub async fn kugou_login_qr_key(app: silvermoon_ipc::Host) -> Result<KugouQrKey, String> {
    silvermoon_ipc::rt::spawn_blocking(move || kugou_login_qr_key_sync(app))
        .await
        .map_err(|e| format!("酷狗请求异常：{e}"))?
}

fn kugou_login_qr_key_sync(app: silvermoon_ipc::Host) -> Result<KugouQrKey, String> {
    ensure_loaded(&app);
...
pub async fn kugou_login_qr_check(
    ...
    silvermoon_ipc::rt::spawn_blocking(move || kugou_login_qr_check_sync(app, key))
        .await
        .map_err(|e| format!("酷狗请求异常：{e}"))?
}

fn kugou_login_qr_check_sync(
    app: silvermoon_ipc::Host,
    key: String,
) -> Result<KugouQrCheck, String> {
    ensure_loaded(&app);
...
/// 发送手机短信验证码
#[silvermoon_ipc::command]
pub async fn kugou_captcha_sent(app: silvermoon_ipc::Host, mobile: String) -> Result<(), String> {
    silvermoon_ipc::rt::spawn_blocking(move || kugou_captcha_sent_sync(app, mobile))
        .await
        .map_err(|e| format!("酷狗请求异常：{e}"))?
}

fn kugou_captcha_sent_sync(app: silvermoon_ipc::Host, mobile: String) -> Result<(), String> {
    ensure_loaded(&app);
...
pub async fn kugou_login_cellphone(
    ...
    silvermoon_ipc::rt::spawn_blocking(move || kugou_login_cellphone_sync(app, mobile, code))
        .await
        .map_err(|e| format!("酷狗请求异常：{e}"))?
}
...
/// 刷新账号信息
#[silvermoon_ipc::command]
pub async fn kugou_account(app: silvermoon_ipc::Host) -> Result<KugouProfile, String> {
    silvermoon_ipc::rt::spawn_blocking(move || kugou_account_sync(app))
        .await
        .map_err(|e| format!("酷狗请求异常：{e}"))?
}

fn kugou_account_sync(app: silvermoon_ipc::Host) -> Result<KugouProfile, String> {
    ensure_loaded(&app);
    let profile = fetch_profile()?;
    {
        let mut st = state().lock().unwrap();
...
#[silvermoon_ipc::command]
pub fn kugou_logout(app: silvermoon_ipc::Host) -> Result<(), String> {
    ensure_loaded(&app);
...
#[silvermoon_ipc::command]
pub async fn kugou_sign_in(app: silvermoon_ipc::Host) -> Result<KugouSignInResult, String> {
    silvermoon_ipc::rt::spawn_blocking(move || kugou_sign_in_sync(app))
        .await
        .map_err(|e| format!("酷狗签到异常：{e}"))?
}

fn kugou_sign_in_sync(app: silvermoon_ipc::Host) -> Result<KugouSignInResult, String> {
    ensure_loaded(&app);
...
pub async fn kugou_cover(url: String) -> Result<String, String> {
    silvermoon_ipc::rt::spawn_blocking(move || kugou_cover_sync(&url))
        .await
