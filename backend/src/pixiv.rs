...
use silvermoon_ipc::{HostApi, WindowBuilder, WindowUrl};
...
fn load_persist(app: &silvermoon_ipc::Host) -> PixivState {
...
fn save_persist(app: &silvermoon_ipc::Host, p: &PixivPersist) -> Result<(), String> {
...
fn persist_current(app: &silvermoon_ipc::Host) -> Result<(), String> {
...
fn apply_token(app: &silvermoon_ipc::Host, raw: &OAuthTokenRaw) -> Result<(), String> {
...
fn refresh_tokens_blocking(app: &silvermoon_ipc::Host) -> Result<(), String> {
    let rt = state()
        .lock()
        .unwrap()
        .refresh_token
...
pub fn pixiv_login_status(app: silvermoon_ipc::Host) -> PixivLoginStatus {
    ensure_loaded(&app);
    let s = state().lock().unwrap();
    PixivLoginStatus {
...
#[silvermoon_ipc::command]
pub async fn pixiv_login_open(app: silvermoon_ipc::Host) -> Result<PixivLoginStatus, String> {
    // 1. 生成 PKCE
    ...
    let build = silvermoon_ipc::rt::spawn_blocking(move || {
        WindowBuilder::new(
            &app2,
            LOGIN_LABEL,
            WindowUrl::External(login_url.parse().unwrap()),
        )
        .title("登录 Pixiv")
        .user_agent(UA)
...
/// 退出登录：清空状态与磁盘
#[silvermoon_ipc::command]
pub async fn pixiv_logout(app: silvermoon_ipc::Host) -> Result<(), String> {
...
/// 推荐（首页）
#[silvermoon_ipc::command]
pub async fn pixiv_recommended(app: silvermoon_ipc::Host) -> Result<PixivIllustPage, String> {
    let q = vec![
...
#[silvermoon_ipc::command]
pub async fn pixiv_refresh_session(app: silvermoon_ipc::Host) -> Result<PixivLoginStatus, String> {
    let app2 = app.clone();
...
#[silvermoon_ipc::command]
pub async fn pixiv_image(_app: silvermoon_ipc::Host, url: String) -> Result<Vec<u8>, String> {
...
#[silvermoon_ipc::command]
pub async fn pixiv_trending_tags(app: silvermoon_ipc::Host) -> Result<Vec<PixivTrendTag>, String> {
...
#[silvermoon_ipc::command]
pub async fn pixiv_frame_bytes(
