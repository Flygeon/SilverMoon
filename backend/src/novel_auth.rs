...
fn load_persist(app: &silvermoon_ipc::Host) -> Wenku8Persist {
...
fn save_persist(app: &silvermoon_ipc::Host, p: &Wenku8Persist) -> Result<(), String> {
...
pub fn current_cookie(app: &silvermoon_ipc::Host) -> Option<String> {
    ensure_loaded(app);
...
/// - `cookies()` 在 Windows 同步命令中**会死锁**（宿主侧已知问题），故必须
...
pub async fn wenku8_login_submit(
    ...
    if !cookie_has_required(&cookie_str) {
        if let Some(w) = app.get_webview_window("wenku8-login") {
            let w2 = w.clone();
            match silvermoon_ipc::rt::spawn_blocking(move || w2.cookies()).await {
                Ok(Ok(cookies)) => {
...
/// 前端据此结束等待（用户取消）。
#[silvermoon_ipc::command]
pub async fn wenku8_login_poll(app: silvermoon_ipc::Host) -> Result<Wenku8LoginStatus, String> {
    ensure_loaded(&app);
    ...
    let Some(w) = app.get_webview_window("wenku8-login") else {
        ...
        // （远程页没有宿主桥，事件接管链走不通，见 wenku8Login.ts 注释）。
        return Err("[WENKU8_LOGIN_CANCELLED] 登录窗口已关闭，未完成登录".into());
    };
    // Windows 下 cookies() 必须在独立线程调用，避免同步命令主线程死锁（同 build 修复）
    let w2 = w.clone();
    let cookies = match silvermoon_ipc::rt::spawn_blocking(move || w2.cookies()).await {
        Ok(Ok(c)) => c,
        Ok(Err(e)) => {
...
/// 返回当前登录状态（不触发网络）。
#[silvermoon_ipc::command]
pub fn wenku8_login_status(app: silvermoon_ipc::Host) -> Wenku8LoginStatus {
    ensure_loaded(&app);
    let s = state().lock().unwrap();
...
#[silvermoon_ipc::command]
pub fn wenku8_logout(app: silvermoon_ipc::Host) -> Result<(), String> {
    ensure_loaded(&app);
    {
        let mut s = state().lock().unwrap();
...
/// 返回已保存的用户信息（若曾拉取过）。
#[silvermoon_ipc::command]
pub fn wenku8_userinfo(app: silvermoon_ipc::Host) -> Option<Wenku8UserInfo> {
    ensure_loaded(&app);
...
(function () {
  function showErr(msg) {
    ...
    try {
      if (window.__SILVERMOON_HOST__) {
        window.__SILVERMOON_HOST__.invoke('wenku8_login_log', { msg: String(msg) }).catch(function(){});
      }
    } catch (e) {}
  }
  ...
  function logCookie(tag) {
    try {
      if (window.__SILVERMOON_HOST__) {
        var s = cookieState();
        window.__SILVERMOON_HOST__.invoke('wenku8_login_log', {
  ...
  function submit() {
    ...
    try {
      if (window.__SILVERMOON_HOST__) {
        window.__SILVERMOON_HOST__.invoke('wenku8_login_log', { msg: '[submit] 调用 wenku8_login_submit（cookie 交给 Rust 校验/兜底读 webview cookie）' }).catch(function(){});
        window.__SILVERMOON_HOST__.invoke('wenku8_login_submit', { cookie: cookie })
          .then(function () {
            try { window.__SILVERMOON_HOST__.invoke('wenku8_login_log', { msg: '[submit] 成功返回，窗口将由 Rust 关闭' }).catch(function(){}); } catch (_) {}
          })
          .catch(function (e) {
            try { window.__SILVERMOON_HOST__.invoke('wenku8_login_log', { msg: 'submit 失败: ' + (e && e.message || e) }).catch(function(){}); } catch (_) {}
          });
      } else {
        showErr('window.__SILVERMOON_HOST__ 未定义，无法提交 cookie（宿主未注入桥接全局）');
      }
...
/// 使用 WindowBuilder.initialization_script（v2 无 eval API，
/// 该方式在每次页面加载前注入，无需额外权限）。
/// 设置浏览器 UA：wenku8 对 WebView2 默认 UA 会返回空白页（参考项目同样设置 Edge UA）。
#[silvermoon_ipc::command]
pub async fn wenku8_login_open(app: silvermoon_ipc::Host) -> Result<(), String> {
    ...
    // 关键修复（白屏/卡死根因）：
    // 同步 command 默认在主线程事件循环内执行，而 WindowBuilder::build()
    ...
    let app2 = app.clone();
    let result = silvermoon_ipc::rt::spawn_blocking(move || {
        silvermoon_ipc::WindowBuilder::new(
            &app2,
            label,
            silvermoon_ipc::WindowUrl::External("https://www.wenku8.net/login.php".parse().unwrap()),
        )
        .title("登录轻小说网 Wenku8")
...
fn wenku8_fetch_userinfo(app: &silvermoon_ipc::Host) -> Result<Option<Wenku8UserInfo>, String> {
