 ...
 * 扩展 UI 在 iframe 内没有宿主桥，通过 postMessage 与本页面通信：
...
async function navigateTo(ext: string, route: string) {
  ...
  loadError.value = "";
  if (!isDesktop) {
    loadError.value = "浏览器预览下无法加载扩展 UI（需要桌面端宿主）";
    return;
  }
  try {
    const base = await appDataDir();
    const htmlPath = await join(base, "extensions", ext, "web", "dist", "index.html");
    iframeSrc.value = `${toAssetUrl(htmlPath)}?ext=${encodeURIComponent(ext)}&route=${encodeURIComponent(route)}`;
  } catch (e) {
